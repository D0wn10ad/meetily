...
async fn run_import<R: Runtime>(
    ...
    for (i, segment) in processable_segments.iter().enumerate() {
        ...
        // Transcribe
        let (text, conf) = if use_funasr {
            let engine = funasr_engine.as_ref().unwrap().clone();
            let samples = segment.samples.clone();
            let text = tokio::task::spawn_blocking(move || {
                engine.transcribe_audio(&samples)
                    .map_err(|e| anyhow!("FunASR transcription failed: {}", e))
            })
            .await
            .map_err(|e| anyhow!("FunASR task panicked: {}", e))??;
            (text, 0.9f32)
...
/// Get or initialize the FunASR engine (OnceLock, initialized once globally)
async fn get_or_init_funasr<R: Runtime>(
    app: &AppHandle<R>,
) -> Result<Arc<FunasrEngine>> {
    if let Some(engine) = IMPORT_FUNASR_ENGINE.get() {
    ...
    info!(
        "Initializing FunASR engine from {}...",
        model_dir.display()
    );

    let engine = FunasrEngine::new(&model_dir)
        .map_err(|e| anyhow!("Failed to initialize FunASR engine: {}", e))?;
    let engine = Arc::new(engine);

    IMPORT_FUNASR_ENGINE.set(engine.clone())
        .map_err(|_| anyhow!("FunASR engine already initialized"))?;
